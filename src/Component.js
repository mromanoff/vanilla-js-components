class Component {
   ...
   */
  sanitize(str) {
    // currently not using this, but it will be useful later.
     ...
     */
  template(props) {
    // this is where our template markup will go.
    let template = `
        <div></div>
      `;
  ...
  render() {
    ...
    let _template =
      typeof this.template === "function" ? this.template(data) : this.template;
